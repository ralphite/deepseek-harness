...
Landlock launcher 以 npm 分发的原生插件（`@deepseek-ai/node-addon-landlock-run`）提供平台 launcher、功能探测与授权词汇。pkg 运行时会先校验嵌入的 launcher 并将其物化到 `$DSH_HOME/cache/native-executables/`，再执行探测。此提供方只做模式到授权的映射，把平台解析与探测解析保留在带版本的 binary 中。
