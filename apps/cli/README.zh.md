...
GitHub Linux 单文件发行版将此启动器安装为 `mydsh`。它保留同一套语法（`mydsh web`、`mydsh --profile headless "job"`），并在启动器和应用帮助中显示该名称。npm、源码和 Python 载体仍使用 `dsh`。单文件发行版的核心 profile 不要求系统 Node.js 或包管理器；`mydsh plugin` 仍要求 pnpm，因为它会按设计转发包管理参数。
